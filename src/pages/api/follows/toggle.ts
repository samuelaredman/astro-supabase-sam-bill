...
export const POST: APIRoute = async (context) => {
  ...
  const { error } = await (db as any).from('follows').insert({ follower_id: profile.id, following_id });
  if (error) return json({ error: error.message }, 400);

  // Fire new_follower notification (non-blocking)
  try {
    const { data: existing } = await (db as any).from('notifications')
...
export const DELETE: APIRoute = async (context) => {
  ...
  const { error } = await (db as any).from('follows').delete().eq('follower_id', profile.id).eq('following_id', following_id);
  if (error) return json({ error: error.message }, 400);
